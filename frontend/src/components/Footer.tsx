...
export default function Footer(){
 ...
 return <footer className="border-t border-gray-800 bg-gray-900 text-gray-300">
  <div className="mx-auto grid max-w-7xl gap-10 px-6 py-12 sm:grid-cols-2 lg:grid-cols-4 lg:px-8">
   ...
   <section><h2 className="mb-4 text-lg font-black text-white">Our Address</h2><p className="mb-3 text-xl font-black text-white">Bhaktivedanta Health Care Center</p><p className="flex items-start gap-3 leading-8"><MapPin className="mt-1 shrink-0 text-sky-500" size={20}/><span>{CLINIC_ADDRESS}</span></p><p className="mt-4 flex items-center gap-3"><Phone className="shrink-0 text-sky-500" size={19}/><a className={linkClass} href={`tel:${CLINIC_PHONE.replace(/\s/g,'')}`}>{CLINIC_PHONE}</a></p><p className="mt-3 flex items-start gap-3 break-all"><Mail className="mt-1 shrink-0 text-sky-500" size={19}/><a className={linkClass} href={`mailto:${CLINIC_EMAIL}`}>{CLINIC_EMAIL}</a></p></section>
