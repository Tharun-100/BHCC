...
const getDoctorWeeklySchedule = (doctor: User): WeeklySchedule => {
...
const generateSlotsFromWindows = (windows: Array<{ start: string; end: string }> = []) => {
  ...
  windows.forEach((window) => {
    ...
    while (currentH < endH || (currentH === endH && currentM < endM)) {
      slots.add(`${currentH.toString().padStart(2, '0')}:${currentM.toString().padStart(2, '0')}`);
      currentM += 15;
      if (currentM >= 60) {
        currentH += 1;
        currentM = 0;
...
const summarizeSchedule = (doctor: User) => {
  const schedule = getDoctorWeeklySchedule(doctor);
  return Object.entries(schedule)
    .filter(([, windows]) => windows && windows.length > 0)
    .map(([day, windows]) => `${day.slice(0, 3)} ${windows.map((window) => `${window.start}-${window.end}`).join(', ')}`)
    .join(' | ');
};
...
const BookAppointment: React.FC = () => {
  ...
  const handleConfirmAndPay = async () => {
    ...
    const appointmentData = {
      ...
      department: selectedDoc.department || selectedDept?.name || '',
      date: selectedDate,
      time: selectedSlot,
    ...
    try {
      ...
      if (result.offlinePayment) {
        setGatewayFee(0);
        setTotalAmount(result.amount);
        setOfflinePaymentMessage(result.message || 'Appointment booked. Please pay at the counter.');
      ...
      router.push(`/payment-gateway?${qs.toString()}`);
    } catch (error) {
  ...
  const availableDates = useMemo(() => {
    ...
    const dates = [];
    const schedule = getDoctorWeeklySchedule(selectedDoc);
    const today = new Date();
    for (let i = 1; i <= 14; i++) {
      const d = new Date();
      d.setDate(today.getDate() + i);
      const dayName = d.toLocaleDateString('en-US', { weekday: 'long' }) as WeekDayName;
  ...
  const availableSlots = useMemo(() => {
    ...
    const dayName = new Date(`${selectedDate}T00:00:00`).toLocaleDateString('en-US', { weekday: 'long' }) as WeekDayName;
    return generateSlotsFromWindows(getDoctorWeeklySchedule(selectedDoc)[dayName] || []);
        ...
        <React.Fragment key={i}>
          <div className="flex flex-col items-center">
            ...
            <span className="text-[9px] mt-2 font-bold uppercase tracking-wider text-center max-w-[40px]">
              {['Dept', 'Doctor', 'Slot', 'Review', 'Confirm', 'Finish'][i - 1]}
            </span>
          </div>
        ...
        <div className="bg-white rounded-[2rem] shadow-xl shadow-gray-200/50 p-8 lg:p-12 min-h-[400px]">
              ...
              {filteredDoctors.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {filteredDoctors.map((doc) => (
                    ...
                    >
                      <div className="w-20 h-20 rounded-xl bg-sky-100 text-sky-700 flex items-center justify-center text-2xl font-black shrink-0">
                        {getDoctorInitials(doc.name)}
                      </div>
                      <div>
                        <h3 className="text-lg font-bold text-gray-900">{doc.name}</h3>
                        <p className="text-sm text-gray-500 mb-1">{doc.specialty}</p>
                        <p className="text-xs text-sky-600 font-bold mb-2">Available: {summarizeSchedule(doc) || 'Schedule not set'}</p>
          ...
          {step === 4 && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="max-w-md mx-auto">
                ...
                >
                  {isProcessing ? <Loader2 className="animate-spin" /> : `Confirm Booking Rs. ${totalAmount}`}
                </button>
              </div>
            </div>
          )}
          ...
          {step === 5 && (
            <div className="text-center py-12 animate-in fade-in duration-700">
              ...
              <h2 className="text-3xl font-extrabold text-gray-900 mb-4">Confirming Appointment</h2>
              <p className="mt-4 font-medium text-gray-600">Please wait while we reserve your consultation slot.</p>
            </div>
          )}
          ...
          {step === 6 && (
            <div className="text-center py-8 animate-in zoom-in duration-500">
              ...
              <p className="text-gray-500 mb-12">{offlinePaymentMessage || 'Your in-person consultation is successfully booked.'}</p>
              ...
              <div className="bg-gray-50 rounded-3xl p-8 max-w-lg mx-auto text-left mb-12 border border-dashed border-gray-300">
                <div className="flex items-center space-x-3 mb-6 p-4 bg-white rounded-2xl">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-900">{CLINIC_ADDRESS}</p>
                  </div>
                </div>
              ...
              <div className="flex flex-col sm:flex-row justify-center space-y-4 sm:space-y-0 sm:space-x-4">
                <button onClick={() => router.push('/dashboard')} className="px-8 py-4 bg-sky-600 text-white rounded-2xl font-bold hover:bg-sky-700 transition">
                  Go to Dashboard
                </button>
