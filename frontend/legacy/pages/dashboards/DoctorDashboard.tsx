          ...
          <div className="bg-gray-900 rounded-3xl p-8 text-white shadow-xl">
            ...
            <div className="space-y-4">
              {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map(day => {
                const isWorking = user.availableDays?.includes(day);
                return (
                  <div key={day} className="flex items-center justify-between">
                    <span className={`text-sm ${isWorking ? 'text-white font-bold' : 'text-gray-600'}`}>{day}</span>
                    <span className={`text-xs px-2 py-1 rounded-lg ${isWorking ? 'bg-sky-500/20 text-sky-400' : 'bg-gray-800 text-gray-700'}`}>
                      {isWorking ? 'In-Clinic' : 'Off'}
                    </span>
                  </div>
                );
              })}
