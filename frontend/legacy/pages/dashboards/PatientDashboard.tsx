...
import { Calendar, Clock, MapPin, FileText, ChevronRight, Activity, Bell } from 'lucide-react';
import { Link } from 'react-router-dom';
import { listPatientAppointments } from '../../services/clinicService';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 space-y-4 md:space-y-0">
        ...
        <Link to="/book" className="px-6 py-3 bg-sky-600 text-white rounded-xl font-bold hover:bg-sky-700 transition flex items-center justify-center">
              ...
              {appointments.filter(a => a.status === 'Upcoming').map(app => (
                <div key={app.id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between group hover:shadow-md transition-shadow">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <button className="px-4 py-2 bg-white border border-gray-100 text-red-500 rounded-xl text-sm font-bold hover:bg-red-50">Cancel</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-gray-900 p-8 rounded-[2.5rem] text-white shadow-xl">
            ...
            <div className="space-y-4 text-sm">
               <div className="flex items-start space-x-3">
                  ...
                  <span>108 Vedanta Lane, Peace Avenue, City-Center 400001</span>
               </div>
